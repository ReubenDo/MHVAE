...
from network.mhvae_mask import MHVAE2D
...
def train(paths_dict, model, netD, device, logger, opt):
    ...
    for phase in PHASES:
        paths_dict[phase] = {mod:paths_dict[phase][mod] for mod in opt.modalities + ['mask']}
    ...
    lr0 = poly_lr(0, opt.epochs, lr0_disct, min_lr=1e-5)
    d_lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
        d_optimizer,
        lr_lambda=lambda epoch: poly_lr(epoch, opt.epochs, lr0_disct, min_lr=1e-5)/lr0)
    ...
    while continue_training:
        ...
        for phase in ['training', 'validation']:
            ...
            for id_b in tqdm(ind_batch[phase]):
                ...
                mask = batch['mask'].permute(0,4,1,2,3).reshape(-1, 1, *batch[mod].shape[2:4])[w:w+nw:,...].to(device).float()
                
                if epoch==1 and id_b==0 and phase=='training':
                    with torch.set_grad_enabled(False):
                        logger.info('[INFO] Display infos latent')
                        model.display_info({mod:imgs[first_mod]}, mask=mask)
                ...
                with torch.set_grad_enabled(phase == 'training'):
                    ...
                    output_img, kls  = model({mod:imgs[mod].clone() for mod in opt.modalities}, mask=mask)
                    ...
                    # Compute for for each modality independently (OK if N=2)
                    for mod in opt.modalities:
                        output_img, kls  = model({mod:imgs[mod]}, mask=mask)
                        kl += torch.sum(torch.stack(kls['prior']))
                        loss_img += nn.SmoothL1Loss(reduction='sum')(output_img, torch.cat([imgs[mod] for mod in opt.modalities], 1))  
                        if w_dis>0:
                    ...
                    # Train Discriminator
                    if phase == 'training' and epoch>opt.warmup_value_null_disc - 10:
        ...
        if epoch%10==0 or epoch<10:
            with torch.no_grad():
                ...
                pred, _  = model({mod:imgs[mod] for mod in opt.modalities}, mask=mask)
                ...
                for mod in opt.modalities:
                    pred, _  = model({mod:imgs[mod]}, mask=mask)
                    pred = 2*((pred+1)/2*(mask>=0).float()) - 1
                    name = batch[f"{mod}_name"][0]+'_'+str(epoch)+'_single.nii.gz'
                    save(pred, affine, os.path.join(saving_path,name))
                
                pred = model.sample(mask=mask)
                pred = 2*((pred+1)/2*(mask>=0).float()) - 1
    ...
    for phase in PHASES:
        ...
        for subject in list_file[phase]:
            ...
            if os.path.exists(opt.path_data+subject+opt.modalities[0]+'.nii.gz'):
                ...
                paths_dict[phase]['mask'].append(opt.path_mask+subject+'mask'+'.nii.gz')
...
def parsing_data():
    ...
    parser.add_argument('--path_mask',
                        type=str,
                        default='../data/golby_data/mask_crop/')
